...
app.post("/api/python", (req, res) => {
  ...
  let number = [];
  req.body.index.map((i) => number.push(Number(i)));
  console.log("인덱스 확인", number);

  const python = spawn("python", [
    "./testyolo/yolov5/y_callDetect.py",
    encodeURI(req.body.data),
    "detectResult",
    req.body.projectId,
    [req.body.index],
  ]);
  ...
  python.on("close", (code) => {
    ...
    const python2 = spawn("python", [
      "./testyolo/S3UpDownLoader/y_call_S3Uploader.py",
    ]);
...
app.post("/api/python2", (req, res) => {
  ...
  const python = spawn("python", [
    "./testyolo/yolov5/y_callDetect_2.py",
    encodeURI(req.body.data),
    "detectResult",
    req.body.projectId,
    [req.body.index],
...
app.post("/api/project/chart", (req, res) => {
  Data.find(
    ...
    { _id: 0, "data.ing": 1, "auto_data.ing": 1 },
    (err, chart) => {
      if (err) console.log(err);
      else {
        return res.status(200).json({
...
app.post("/api/datatxt", (req, res) => {
  ...
  myBucket.putObject(params).send((err, nn) => {
    if (err) {
      console.log(err);
      return err;
    }
...
app.post("/api/data/draw", (req, res) => {
  ...
  var params = {
    Bucket: S3_BUCKET,
    Key:
      req.body.label + "/" + req.body.projectId + "/labels/" + imageId + ".csv",
  };
...
app.post("/api/overview/todolist", (req, res) => {
  ...
  Project.updateOne(
    { _id: req.body.id, "users._id": Number(req.body.Id) },
    { $addToSet: { "users.$.todo": { $each: req.body.todo } } },
    (err, document) => {
      if (!document) {
        console.log(err);
      } else {
...
app.post("/api/overview/todo_delete", (req, res) => {
  Project.updateOne(
    { _id: req.body.id, "users._id": Number(req.body.Id) },
    { $pullAll: { "users.$.todo": req.body.todo } },
    (err, document) => {
      if (!document) {
        console.log(err);
      } else {
...
app.post("/api/overview/todoview", (req, res) => {
  Project.find(
    { $and: [{ _id: req.body.id }, { "users._id": req.body.Id }] },
    (err, document) => {
      if (!document) {
        console.log(err);
      }
...
app.post("/api/datas/ingUpdate", (req, res) => {
  ...
  if (req.body.label == "manual") {
    Data.update(
      { _id: req.body.idx, "data.name": req.body.data_id },
      { $set: { "data.$.ing": "작업 완료" } },
      (err) => {
    ...
    Data.update(
      { _id: req.body.idx, "auto_data.name": req.body.data_id },
      { $set: { "auto_data.$.ing": "작업 완료" } },
      (err) => {
...
app.post("/api/data/delete", (req, res) => {
  ...
  const img_params = {
    Bucket: S3_BUCKET,
    Key:
      req.body.label + "/" + req.body.projectId + "/" + req.body.name + ".jpeg",
  };
  if (req.body.label == "manual") {
    Data.updateOne(
      { _id: req.body._id, "data.name": req.body.name },
    ...
    Data.deleteMany(
      { _id: req.body._id, "auto_data.name": req.body.name },
      { $pull: { auto_data: { name: req.body.name } } },
      (err, cc) => {
...
const port = process.env.PORT || 5001; // 5000번 포트를 백서버로 둔다
